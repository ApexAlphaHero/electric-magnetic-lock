...
- **The Hardware page changes NO/NC polarity, not physical wiring.** `lock.active_low_relay` and `doors[].active_low` are the same fields in `config.json`; the page just lets an operator flip them without a file edit and a restart. `set_lock_polarity`/`set_door_polarity` (control socket, `_run_web_command`) persist via `save_config` and then call `LockController.set_active_low`/`DoorSensor.set_active_low` to re-apply live — the lock re-drives the relay under its *current* logical state (so flipping polarity never itself locks/unlocks the door) and a sensor re-reads its live GPIO level and republishes `DOOR_STATE` if the reinterpreted level changed. The lock's setting is deliberately **not** framed as the maglock's own NO/NC: that dry-contact wiring is fixed at COM/NC (see Hardware section) and unaffected by this — it only matches whichever relay module's trigger polarity was wired in.
